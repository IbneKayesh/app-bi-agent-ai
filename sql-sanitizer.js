...
try {
  ...
  console.warn('Could not load database-schema.json for sql-sanitizer:', err.message);
  schema = null;
}
...
if (schema && schema.database && Array.isArray(schema.database.tables)) {
  for (const t of schema.database.tables) {
    tableMap[t.table_name] = Array.isArray(t.columns) ? t.columns.slice() : [];
  }
}
...
function quoteColumn(table, col) {
  if (!isValidColumn(table, col)) throw new Error(`Column not allowed: ${col} on ${table}`);
